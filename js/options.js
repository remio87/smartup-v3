	...
	createMoreCheck: function (_conf) {
		...
		var dom = suo.domCreate2("div", { setName: ["className"], setValue: ["box_optionlist"] }),
			...
			domCheck = suo.domCreate2("input", {
				setName: ["id", "className", "type", "name"], setValue: [conf.type, "box_check", "checkbox", conf.type]
			});
			...
			switch (conf.typeCheck) {
				...
				case "text":
					var domText = suo.domCreate2("input", {
						setName: ["name", "type", "value", "className"], setValue: [conf.type, "text", conf.valueOption == undefined ? actionOptions.checks[conf.type].options.valueOption : conf.valueOption, "box_checktext"]
					});
